...
const DEFAULT_SESSIONS_DIR = '~/.glove/sessions';

/**
 * glove — passive monitoring of sandboxed harnesses. Enabling it points the
 * existing file watcher at glove's per-sandbox homes in addition to the native
 * one; native monitoring is unaffected either way. Read-only: nothing is written
 * into a sandbox. See CLAUDE.md "Type duplication" — mirrors GloveConfigSchema.
 */
  ...
  return (
    <>
      ...
      <ToggleRow
        ...
        desc="Tail harness logs from glove sandboxes alongside native sessions. Read-only; only harnesses that persist a transcript (Mistral Vibe) are discovered. Sandboxed sessions are tagged with their sandbox name."
      ...
      <CustomRow>
        ...
        <span style={{ fontSize: 10.5, color: 'var(--text-faint)', lineHeight: 1.5 }}>
          Host directory glove persists sandbox homes under; each is scanned at
          <code style={{ margin: '0 3px', fontFamily: 'var(--font-mono)' }}>&lt;dir&gt;/&lt;name&gt;/home/</code>.
          In Docker this must match the mount in docker-compose.yml (default maps to the container's <code style={{ fontFamily: 'var(--font-mono)' }}>~/.glove/sessions</code>).
        </span>
      </CustomRow>
    </>
  );
