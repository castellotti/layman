...
- **History import discovers glove pi sessions too** (`recovery.ts`, `transcript-pi.ts`).
  ...
  imported from a sandbox: claude-code isn't run under glove, and Vibe has no history importer at
  all — so non-pi roots are filtered out, and `gloveRoots` is empty when glove is disabled, leaving
