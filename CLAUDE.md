...
4b. **Monitor sources** (`packages/server/src/monitor/sources.ts`): A `MonitorSource` enumerates `WatchRoot`s (a `.../logs/session` dir plus the agent type and an optional sandbox `label`) for the passive watcher, on demand. `NativeVibeSource` returns the historical single `~/.vibe` root; `GloveSource` globs `~/.glove/sessions/*/home/.vibe/logs/session` and returns one labelled root per sandbox that actually contains Vibe logs. Native precedes glove in the list, so it wins any path collision. Only Vibe is passive today, so every root declares `agentType: 'mistral-vibe'`; the interface separates *where* to watch (a source) from *how* to parse (the watcher's Vibe-specific logic), so a future passive harness adds a source and a parser branch and nothing else changes.
